...
export default function DesignWorkspacePage() {
    ...
    const [selectedCategoryFilter, setSelectedCategoryFilter] = useState<string | null>(null); // null means "All"

    // AI Iterations
    const [aiGeneratedIterations, setAiGeneratedIterations] = useState<Array<{
        id: string;
        label: string;
        img: string;
        timestamp: string;
    ...
    const toggleReference = (imgId: string) => {
        setSelectedReferences(prev =>
            prev.includes(imgId)
                ? prev.filter(i => i !== imgId)
                : [...prev, imgId]
        );
    };

    // Compute Market Comps groups
    const groupedComps = compsImages.reduce((acc, img) => {
    ...
    const marketComps = Object.entries(groupedComps).map(([name, images], index) => ({
        id: `comp-${index}`,
    ...
    const uniqueCategories = Array.from(new Set(allImages.map(img => img.category || 'uncategorized')))
        .sort();

    // Filter images based on selected category
    const filteredMlsImages = selectedCategoryFilter
        ? mlsImages.filter(img => (img.category || 'uncategorized') === selectedCategoryFilter)
        : mlsImages;

    const filteredCompsImages = selectedCategoryFilter
        ? compsImages.filter(img => (img.category || 'uncategorized') === selectedCategoryFilter)
        : compsImages;
                    ...
                    <div className="flex-1 flex flex-col gap-6 h-full overflow-y-auto pr-2 pb-10 custom-scrollbar">
                        ...
                        {uniqueCategories.length > 0 && (
                            <div className="bg-white rounded-xl border border-border p-4 shadow-sm">
                                <h3 className="architectural-label text-primary mb-3">Filter by Category</h3>
                                <div className="flex flex-wrap gap-2">
                                    ...
                                    {uniqueCategories.map((category) => {
                                        const count = allImages.filter(img => (img.category || 'uncategorized') === category).length;
                                        const displayLabel = category === 'uncategorized'
                                            ? 'Uncategorized'
                        ...
                        <div className="bg-white rounded-xl border border-border p-4 shadow-sm">
                            <div className="flex items-center justify-between mb-4">
                                <h3 className="architectural-label text-primary">Subject Property (MLS)</h3>
                                <span className="text-xs text-muted-foreground">{filteredMlsImages.length} images</span>
                            </div>
                            ...
                            <div className="grid grid-cols-3 gap-4">
                                {filteredMlsImages.map((img) => {
                                    ...
                                    const caption = img.caption || img.category || "Uncategorized";
                                        ...
                                        >
                                            <div className="aspect-[4/3] bg-secondary relative">
                                                ...
                                                <div className={cn(
                                                    ...
                                                    isSelected ? "border-primary text-primary opacity-100" : "border-muted-foreground opacity-0 group-hover:opacity-100"
                                                )}>
                                            ...
                                            <div className="p-2 bg-white border-t border-border">
                                                <p className="text-xs font-medium truncate" title={caption}>
                                                    {caption}
                                                </p>
                                            </div>
                                        </div>
                        ...
                        {aiGeneratedIterations.length > 0 && (
                            <div className="bg-white rounded-xl border border-border p-4 shadow-sm">
                                <h3 className="architectural-label text-primary mb-4">Generated Iterations</h3>
                                    ...
                                    {aiGeneratedIterations.map((item) => (
                                        <div key={item.id} className="group relative rounded-lg overflow-hidden border border-border">
                                            ...
                                            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors flex items-center justify-center opacity-0 group-hover:opacity-100">
                                                <Button size="sm" variant="secondary" className="h-7 text-xs" onClick={() => window.open(item.img, '_blank')}>
                        ...
                        <div className="bg-white rounded-xl border border-border p-4 shadow-sm">
                            <h3 className="architectural-label text-primary mb-4">Market Comps</h3>
                            {(() => {
                                // Regroup filtered comps
                                const filteredGroupedComps = filteredCompsImages.reduce((acc, img) => {
                                    const key = img.filename.split('_')[0] || 'Reference';
                                    if (!acc[key]) acc[key] = [];
                                    acc[key].push(img);
                                    return acc;
                                ...
                                const filteredMarketComps = Object.entries(filteredGroupedComps).map(([name, images], index) => ({
                                    id: `comp-${index}`,
                                    title: name.length > 20 ? name.substring(0, 20) + '...' : name,
                                    match: "Ref",
                                    assets: images
                                                    ...
                                                    >
                                                        ...
                                                        {caption && (
                                                            <div className="absolute bottom-0 left-0 right-0 bg-black/60 backdrop-blur-sm px-2 py-1">
                                                                <p className="text-[10px] text-white truncate" title={caption}>
                                                                    {caption}
                                                                </p>
                                                            </div>
                                                        )}
